...
(function () {
  ...
  function syncNotificationBadge() {
    ...
    let feed = null;
    try {
      feed = JSON.parse(localStorage.getItem("todayFridgeNotificationsFeed") || "null");
    } catch (_) {
      feed = null;
    }
    const hasAuthenticatedUser = Boolean(localStorage.getItem("todayFridgeBoundUserId"));
    const hasUnread = Array.isArray(feed)
      ? feed.some((item) => item && item.read !== true)
      : !hasAuthenticatedUser;
    dot.hidden = !hasUnread;
    dot.parentElement?.setAttribute(
      "aria-label",
      hasUnread ? "읽지 않은 알림 보기" : "알림 보기"
    );
  }
  ...
  function bindMainButtons() {
    ...
    document.getElementById("btn-guide")?.addEventListener("click", () => window.showAppModal("modal-guide"));
